...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        print("create Model from: ",opt.cfg)
    ...
    logger.info('Image sizes (%g,%g) train, (%g,%g) test\n'
                ...
                'Starting training for %g epochs...' % (imgsz[0], imgsz[1],imgsz_test[0], imgsz_test[1], dataloader.num_workers, log_dir, epochs))
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Multi-scale
            if opt.multi_scale:
                sz = random.randrange(imgsz[0] * 0.5, imgsz[0] * 1.5 + gs) // gs * gs  # size
                # sz = (imgsz * 1.5 + gs) // gs * gs  # size
                # sf = sz / max(imgs.shape[2:])  # scale factor
                sf = np.min([sz/imgs.shape[2], sz * imgsz[1] / (imgsz[0] * imgs.shape[3])])
                if sf != 1:
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                torch.save(ckpt, wdir / '{:d}.pth'.format(epoch))
                del ckpt
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', nargs='+', type=int, default=[[384,640], [384,640]], help='[train, test] image sizes (h,w)')
