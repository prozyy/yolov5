...
vid_formats = ['.mov', '.avi', '.mp4', '.mpg', '.mpeg', '.m4v', '.wmv', '.mkv', '.flv','.h264']
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=[384,640], batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            ...
            ratio_g = img_size[0] / img_size[1]
            # Set training image shapes
            shapes = [[1, 1 / ratio_g]] * nb
            for i in range(nb):
                ari = ar[bi == i]
                mini, maxi = ari.min(), ari.max()
                if maxi < ratio_g:
                    shapes[i] = [maxi / ratio_g, 1 / ratio_g]
                elif mini > ratio_g:
                    shapes[i] = [1, 1 / mini]
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        r = np.min([self.img_size[1] / w0, self.img_size[0] / h0])
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s[0] * 2, s[1] * 2, img.shape[2]), 0, dtype=np.uint8)  # base image with 4 tiles
    ...
    if len(labels4):
        ...
        np.clip(labels4[:, 1::2], 0, 2 * s[1], out=labels4[:, 1::2])  # use with random_perspective
        np.clip(labels4[:, 2::2], 0, 2 * s[0], out=labels4[:, 2::2])
