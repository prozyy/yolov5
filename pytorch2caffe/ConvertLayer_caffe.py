...
def flatten(pytorch_layer):
    ...
    layer = pb2.LayerParameter()
    layer.type = "Dropout"
    layer.dropout_param.dropout_ratio = float(0.5)
    train_only = pb2.NetStateRule()
    train_only.phase = pb2.TRAIN
...
def UpsampleBilinear(pytorch_layer):
    ...
    assert pytorch_layer.scale_factor[0] == pytorch_layer.scale_factor[1], 'UpsampleBilinear scale error {}'.format(pytorch_layer.scale_factor)
    factor = int(pytorch_layer.scale_factor[0])
    c = 256
    if pytorch_layer.output_size[1] == 40:
        c = 256
    if pytorch_layer.output_size[1] == 80:
        c = 128
    # c = 32*48//int(pytorch_layer.output_size[1])
    #c = 8*40//int(pytorch_layer.output_size[1])
    k = 2 * factor - factor % 2
    ...
    layer.blobs.extend([as_blob(blobs_weight)])
    layer.blobs.extend([as_blob(np.zeros(shape=(c)))])
...
def MaxPooling(pytorch_layer):
    ...
    layer.pooling_param.kernel_size = 3
...
def AvgPooling(pytorch_layer):
    ...
    # print(pytorch_layer.kernel_size)
    layer = pb2.LayerParameter()
    layer.type = "Pooling"
