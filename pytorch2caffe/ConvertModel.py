...
def DFS(func):
    scale_table= open('scale.table','a')
    if func in visited:
    ...
    if dst == 'caffe':
        if layer_type_name in ['Clone', 'Threshold', 'Dropout', 'SetItem', 'BatchNorm']:
	...
	# scale_table.write(name.replace('BatchNorm','ConvNd')+" "+str(int(func.eps))+'\n')
	#print(name.replace('BatchNorm','ConvNd')+" "+str(int(func.eps)))
...
def ConvertModel_caffe(pytorch_net, InputShape, softmax=False, use_cuda=False, save_graph=False):
    ...
    if use_cuda:
        ...
        inputs = Variable(torch.ones(n, c, h, w).cuda(), requires_grad=True)
    else:
        inputs = Variable(torch.ones(n, c, h, w), requires_grad=True)
